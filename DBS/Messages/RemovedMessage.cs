...
{
    public class RemovedMessage : Message // <Version> <FileId> <ChunkNo> <CRLF> <CRLF>
    {
        public RemovedMessage(FileChunk fileChunk)
            : this(Core.Instance.Config.VersionM, Core.Instance.Config.VersionN,
            fileChunk.FileId, fileChunk.ChunkNo)
        { }

        public RemovedMessage(int versionM, int versionN, FileId fileId, int chunkNo)
            : base(MessageType.Removed)
        {
        ...
        {
            return string.Format("{0} {1} {2}#{3}", MessageType,
                GetVersion(), FileId.ToStringSmall(), ChunkNo);
        }
        ...
        public new static Message Deserialize(byte[] data)
        {
            using (var stream = new MemoryStream(data))
            {
                ...
                try
                {
                    return new RemovedMessage(versionM, versionN, fileId, chunkNo);
                }
                catch (Exception ex)
                {
                    Core.Instance.Log.Error("Could not create RemovedMessage", ex);
                    return null;
                }
            }
        }
